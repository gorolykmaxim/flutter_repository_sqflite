# flutter_repository_sqflite

Implementation of flutter_repository collections, that houses data in an 
sqlite database.

## Getting Started

The example is quite long, though at the end of it you will a working
domain and persistence models.

Let's create our Tweet and Comment entities:
```dart
class Comment {
  int id;
  int tweetId;
...
To simplify the process of their construction, lets make factories
for them:
```dart
...
To let sqflite repository know how to persist tweets and comments in the
database, lets create data servants for them:
```dart
...
All set! We can now initialize our application:
```dart
